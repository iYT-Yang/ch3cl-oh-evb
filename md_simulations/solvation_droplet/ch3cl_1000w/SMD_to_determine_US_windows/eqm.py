...
for i in range(5000):
    if i % 5 == 2:  # For numbers that are 2, 7, 12, 17, 22, ... which are OM of water (no mass)
        continue  # if the particle is OM of water (no mass), do not add it as a particle group
    wall.addGroup([i])
...
restraint: RESTRAINT ARG=dist.z AT=40 KAPPA=8.368
...
PRINT ARG=dist.z,restraint.bias STRIDE=10000 FILE=eqm.cv
"""
